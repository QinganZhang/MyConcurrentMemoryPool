...
public:
	...
	explicit TCMalloc_PageMap1()
	{
		...
		array_ = (Span**)SystemAlloc(alignSize >> PAGE_SHIFT); //向堆申请空间
    ...
    const Span* operator[] (Number k) const {
		if ((k >> BITS) > 0) //k的范围不在[0, 2^BITS-1]
		{
			return nullptr;
		}
    ...
    Span* end() {return (Span*)nullptr;}
    Span* find(Number k) const {
        return get(k);
    }

private:
	Span** array_; //存储映射关系的数组
...
{
private:
	static const int ROOT_BITS = 6;                //第一层对应页号的前6个比特位
